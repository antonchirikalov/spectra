...
Два Python-**оркестратора** управляют пайплайнами LLM-агентов. При старте оркестратор поднимает один общий процесс **`opencode serve`** и общается с ним по **HTTP REST + SSE** — каждый шаг агента выполняется как отдельная сессия на этом сервере: никаких процессов на шаг, живой прогресс по server-sent events, чистая отмена через API. Сами агенты — markdown-определения в `.opencode/agents/`, работают на любой модели любого провайдера (Kimi K3, DeepSeek, GPT, Claude, Qwen или локальная модель через Ollama). Модели назначаются декларативно в **`models.yaml`** (обязательный `default_model`, закрепления за агентами, N кандидатов-дизайнеров для Solution Design). Все передачи между фазами — файлы на диске, каждый шаг фиксируется в атомарном **`state.json`**-журнале — прерванный прогон возобновляется ровно с места остановки. Legacy-транспорт `subprocess` (по процессу `opencode run` на шаг) остаётся как флаг `--transport subprocess`.
...
Единое место назначения моделей для обоих оркестраторов. Файл в `.gitignore` (локальный конфиг) — создаётся один раз: `cp models.yaml.example models.yaml`. **Обязателен**: `run`/`resume` не стартуют без него (`default_model` должен быть задан — захардкоженного дефолта нет). Приоритет:

- `requirements_runner`: `plan/params.yaml` прогона → CLI `--model` → `models.yaml`
- `solution_design_runner`: CLI `--models` → `models.yaml`

```yaml
default_model: kimi/kimi-k3          # обязателен — дефолт для всего, что не закреплено ниже
